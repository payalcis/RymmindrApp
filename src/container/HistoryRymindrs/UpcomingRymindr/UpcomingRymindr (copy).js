...
const UpcomingRymindr = (props) => {
  ...
  const handleDelete = (recr_rymindr_id, rymindr_id, qiscus_id) => {
    setOpenDelete(true);
    let getIndex = historyrymindr.upcomingList.findIndex((x) => x.rymindr_id == rymindr_id);
    let upcoming = historyrymindr.upcomingList[0].rymindr_id;
    if (historyrymindr.upcomingList[getIndex + 1]) {
      upcoming = historyrymindr.upcomingList[getIndex + 1].rymindr_id;
    }
    const setdata = {
  ...
  useEffect(() => {
    console.log('historyrymindr=22222=',historyrymindr);
    if(historyrymindr){
      setListLoader(false);
    }
  ...
  const handlesearch = (e) => {
    ...
    console.log('historyrymindr.upcomingList====',historyrymindr.upcomingList);
    ...
    // } else {
    //   setUpcomingRymindr(historyrymindr.upcomingList);
    // }
  ...
  const Rymindrlist =
    historyrymindr !== null && historyrymindr.upcomingList.length > 0 ? (
      historyrymindr.upcomingList.map((item, index) => (
        <>
      ...
      <ListItem button>
        <ListItemText primary={'Nothing in History.'} secondary="" />
      </ListItem>
    );
  ...
  const clearButton =
    historyrymindr !== null && historyrymindr.upcomingList.length > 0 ? (
      <Button
        ...
        color="inherit"
        size="large"
        onClick={() => clearHistoryFn(historyrymindr.rymindr_id)}
